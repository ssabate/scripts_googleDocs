...
function demanarDades(nomFitxer,celaInicial, celaText){
  ...
  //Si s'ha cancelat mostro missatge
  if(cancelat) SpreadsheetApp.getUi().alert('Procés cancel·lat per l\'usuari');
}
...
function crearComentaris(nomFitxer,celaInicial, celaText){
  ...
  for(var i in activeRow){
    ...
    //pagina.setColumnWidth(1,200);
    //Fem que la columna ajuste l'amplada al seu contingut
    //pagina.autoResizeColumn(1);    
    
    var fila=1;
    // Recorrem les diferents columnes    
    for(var col=1;col<numberOfColumns;col++){
      // Posem els títols de les columnes
      pagina.setActiveRange(pagina.getRange(fila++,1));
      pagina.getActiveRange().setWrap(true);
      pagina.getActiveRange().setFontSize(14);
      pagina.getActiveRange().setBackground("lightgreen");
      ...
      pagina.getActiveRange().setWrap(true);
      pagina.getActiveRange().setFontSize(14);
      pagina.getActiveRange().setBackground("white");      
    ...
    pagina.setActiveRange(pagina.getRange(1,1));
    pagina.getActiveRange().setBackground("white");   
